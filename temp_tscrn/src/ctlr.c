...
data_dispatcher_subscribe_t sbscr_temp_meas = {
    .callback = changed_temperature,
};
data_dispatcher_subscribe_t sbscr_temp_setting = {
    .callback = changed_setting,
};
data_dispatcher_subscribe_t sbscr_ctlr_setting = {
...
void changed_temperature(const data_dispatcher_publish_t *data)
{
    const data_dispatcher_publish_t *ctlr_data;
    ...
    switch (ctlr_data->controller.mode) {
        case DATA_CTLR_ONOFF:
            {
                onoff_ctrl(data, NULL, ctlr_data, loc);
            }
            break;
...
void changed_setting(const data_dispatcher_publish_t *data)
{
    const data_dispatcher_publish_t *ctlr_data;
    data_loc_t loc = data->loc;
    ...
    switch (ctlr_data->controller.mode) {
        case DATA_CTLR_ONOFF:
            {
                onoff_ctrl(NULL, data, ctlr_data, loc);
            }
            break;
...
void changed_ctlr(const data_dispatcher_publish_t *data)
{
    data_loc_t loc = data->loc;

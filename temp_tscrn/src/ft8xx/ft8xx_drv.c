...
#define NODE_ID DT_INST(0, ftdi_ft800)
...
// TODO: Remove this semaphor. It is redundant with SPI
...
int ft8xx_drv_init(void)
{
    ...
    cs_ctrl = (struct spi_cs_control){
        .gpio_dev = device_get_binding(DT_SPI_DEV_CS_GPIOS_LABEL(DT_NODELABEL(ft800))),
        .gpio_pin = DT_SPI_DEV_CS_GPIOS_PIN(DT_NODELABEL(ft800)),
        .gpio_dt_flags = DT_SPI_DEV_CS_GPIOS_FLAGS(DT_NODELABEL(ft800)),
        .delay = 0,
    };
    ...
    // TODO: Verify if such entry in DTS is present. If not, use polling mode.
    int_ft8xx_dev = device_get_binding(DT_GPIO_LABEL(NODE_ID, irq_gpios));
    if (!int_ft8xx_dev) {
        return -EINVAL;
    }

    ret = gpio_pin_configure(int_ft8xx_dev,
                       IRQ_PIN,
    ...
    ret = gpio_pin_interrupt_configure(int_ft8xx_dev,
                       IRQ_PIN,
...
{
    // TODO: prevent mutual access to make this function reentrant
	int ret;
	...
	struct spi_buf tx[] = {
		...
		{
			.buf = (uint8_t *)data, // Discard const, because it is implicit for TX buffer
			.len = length,
		},
	};
